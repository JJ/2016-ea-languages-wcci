...
\title{A comparison of implementation of basic evolutionary algorithms
  functions in different languages} 
% \title{How different programming languages affect the performance in evolutionary algorithm experiments}
...
\begin{abstract}
  ...
  That is why in this paper three evolutionary algorithm operations have been chosen: 
  bitflip mutation, crossover and the OneMax evaluation fitness function, and have 
  been measured the speed for both popular and not so popular languages
  in order to determine how these programming languages affect the performance in evolutionary algorithm experiments. 
  The results confirm that Java, C and C++ not only are the fastest, but also
...
\begin{table}[htb]
    ...
    \begin{tabular}{l|c|l|l}
      ...
      \hline
      Scala & 2.11.7 & http://git.io/vBSYH & String, Bit Vector \\
      Lua & 5.2.3 & http://git.io/vBSY7 & String \\
      Perl & v5.20.0 & \url{git.io/bperl} & String, Bit Vector \\
      JavaScript & node.js 5.0.0 & http://git.io/vBSYd & String \\
      Python & 2.7.3 & http://git.io/vBSYb & String\\
      Go & go1.2.1 & http://git.io/vBSYp & Bit Vector \\
      Julia & 0.2.1 & http://git.io/vBSOe & Bit Vector \\
      C & 4.8.2 & http://git.io/v8T57 & char vector \\
      C++ & 4.8.4 & http://git.io/v8T57 & bitset  \\
      Java & 1.8.0\_66 & \url{http://git.io/v8TdR} & Bitset \\
      PHP & 5.5.9 & \url{http://git.io/v8k9g} & String \\ % vrivas, 5-nov-2015
      Rust &  1.4.0  & \url{https://github.com/JJ/RustEO} & BitVector \\ 
      Haskell & ghc 7.10.3 & \url{http://git.io/v8T57} & Mutable Vector \\ 
      Octave & 3.8.1 & http://git.io/v8T57 & BitVector \\ 
      Clojure & 1.8.0 & http://git.io/v8T57 & Persistent Vector \\ 
      Common Lisp & 0.13.7 & http://git.io/v8T57 & Simple Bit Vector \\ 
      \hline
      \end{tabular}
...
%
    ...
    Compiled languages are represented by Rust, Scala, Java, Go, C and C++. Scala and Java both use Java bytecode and run in the Java Virtual Machine. Go, C and C++ compile to a binary that runs natively on the target operating system. Also JavaScript is compiled to native machine code when it runs in the V8 JavaScript Engine used by both Node.js and Google Chrome web browser.  This list of languages is rather comprehensive, including the most popular compiled languages in scientific computing as well as two languages that are emerging in popularity: Go and Scala.
