...
fastest can be several orders of magnitude, up to 5. 
...
for the crossover operator in Figure \ref{fig:time:xo}. The fastest overall are Go, C\# and,
once again, Java, but C++ is among the slowest, on a par with Scala (if it uses a BitString). The scaling of
...
this kind of operation. But it is also interesting to note that, in this case, using contant structures can result in faster operation, as in the case of Scala. 

If we proceed to the last benchmark, the OneMax function shown in Figure \ref{fig:time:mo} we see that the behaviour for all
