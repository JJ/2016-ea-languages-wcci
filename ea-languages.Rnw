...
\and
\IEEEauthorblockN{Amaury Hernández-Águila}
\IEEEauthorblockA{Tijuana Institute of Technology (Mexico)\\
...
This is, as a matter of fact, the state of the art such as we have found them. That is why we think it is important to take real measures so that decisions on implementation are based on facts strictly related to EAs, instead of relying on common lore.
...
The memory and speed efficiency of these data structures is different, and it is advisable for anyone implementing an EA to check all possible data structures before committing, out of inertia, to the easiest one. Once again, implementation matters \cite{DBLP:conf/iwann/MereloRACML11,nesmachnow2011time}, and differences in EA performance for different data structures can be, indeed, quite big.
...
%
    ...
    Compiled languages are represented by Rust, Scala, Java, Common Lisp, Clojure, Go, C, C\# and
    C++. Scala, Clojure and Java use Java bytecode and run in the Java
    Virtual Machine.  C\# runs in the Common Language Runtime. Go, Common Lisp, C and C++ compile to a binary that runs
...
usual functions found in EA frameworks: mutation, crossover  % "found"  =>  "used"
...
any EA. However, for these atomic functions
