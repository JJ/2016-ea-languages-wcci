...
\item Generate a random integer from 0 to the length of the chromosome. %Pablo: Generating
...
\begin{table}[htb]
    ...
    \end{table}
%    
The compiled languages are presented by Scala, a strongly-typed %Pablo: no se quejarán de que no hayamos usado Java directamente? También: presented o represented?
