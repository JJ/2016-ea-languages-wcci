...
\begin{abstract}
  ...
  practice or on usual frameworks using mainstream languages. % maybe "or on usual frameworks used in mainstream languages"
  That is why  it is usual to rely on compiled languages (namely Java 
  or C/C++) when implementing evolutionary algorithms, without 
  ...
  popular computer languages have been used. 
...
fastests, fast enough, and also within the set of languages that are
...
implemantation of basic EA operators in different languages could be useful
to researchers or students that are learning to program, or evaluationg different
technologies. 
