...
\begin{abstract}
  ...
  practice or on usual frameworks using in mainstream languages. That
  is why  it is usual to rely on compiled languages (namely Java or
  C/C++) when implementing, without 
  ...
  bitflip mutation, crossover and the OneMax fitness
  function. Performance tests
  have been the speed for both popular and not so popular computer
  languages. In general, the results confirm that compiled languages
  ...
  (interpreted) are fast enough for 
