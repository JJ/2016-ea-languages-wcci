...
operations: mutation, crossover and onemax. 

We can conclude that the language usually considered the fastest,
...
performance of fast languages such as Scala will disappear. 
...
should be expected. Haskell, on the other hand, has a fast
performance, but it should be noted that in a particular case we
...
that is superior to all others across all sizes, problems and data
