...
\begin{abstract}
  ...
  of the operands, the version of the language and underlying factors such as the operating system. However, it is usual to rely on compiled languages,
...
This escenario is even more blurred if we look at the results for crossover in Figure \ref{fig:time:xo}. The fastest overall is Go, and the slowest is C++ at least for sizes bigger than 64. The scaling of this performance is similar for C, which becomes the second slowest for sizes bigger than 1024. The node JavaScript interpreter performs quite consistently and independently of size, but Perl and PHP are also quite fast for a significant portion of the size range. 
