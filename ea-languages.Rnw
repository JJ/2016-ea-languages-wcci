...
\and
\IEEEauthorblockN{Israel Blancas-Álvarez}
\IEEEauthorblockA{Computer Architecture and Technology Department\\
University of Granada (Spain)\\
Email: iblancasa@ugr.es}
\and
\IEEEauthorblockN{Pedro A. Castillo}
\IEEEauthorblockA{Computer Architecture and Technology Department\\
University of Granada (Spain)\\
Email: pacv@ugr.es}
\and
\IEEEauthorblockN{Gustavo Romero}
\IEEEauthorblockA{Computer Architecture and Technology Department\\
University of Granada (Spain)\\
...
\begin{abstract}
  ...
  different operations in order to choose the best language for a
  single or multilanguage implementation, relying instead on common
  practice or on usual frameworks using in mainstream languages. % maybe "or on usual frameworks used in mainstream languages"
  That is why  it is usual to rely on compiled languages (namely Java 
  or C/C++) when implementing, without 
  considering other languages or rejecting them outright on the basis
  of performance. Since there are a myriad of languages nowadays, we
  considered an interesting challenge to measure their speed when
  performing usual operations in evolutionary algorithms. In this
  paper we have tested three basic evolutionary algorithm operations
  over binary chromosomes:  % have been chosen: 
  bitflip mutation, crossover and the OneMax fitness function. 
  %Performance tests have been the speed for both popular and not so popular computer languages
  % Maybe we could write this previous sentence as follows:
  As a performance measure, the speed for both popular and not so
  popular computer languages has been used. 
  In general, the results confirm that compiled languages
  scale and perform better, but also in some cases  have a behaviour
  that is independent of the size of the chromosome. Additionally,
  results show that other languages, such as Go (compiled) or Python
  (interpreted) are fast enough for most purposes.  % Should we indicate a % of speedup?
  % we would have to create a ránking - JJ
  Besides, these experiments show which of these operations are,  % "measures" ? 
  in fact, the best for choosing an implementation language based on
  its performance. 
\end{abstract}
...
Most released evolutionary algorithms implementations, and also most
...
a set of languages. Past experience or simply know-how in teams will
also make some languages, outside the ones mentioned above, have a
different set of languages to choose for our implementation. Besides,
...
fastests, fast enough, and also within the set of languages that is
used nowadays. Besides, it might be the case that some emerging
language whose popularity in the area of evolutionary algorithms is
...
baseline for evolutionary algorithms will also be used: OneMax. 
...
language to implement that part of your evolutionary algorithm. This makes the
...
not to optimize the total running time of evolutionary algorithms by
...
be considered by EA (evolutionary algorithm) practitioners. We were  % this acronym should be defined before, and used along the rest of the text 
% Do it! - JJ
...
of evolutionary algorithm implementations. Next we will present in
Section \ref{sec:exp} 
...
languages running some widely used evolutionary algorithm operators
and functions 
...
environments reviewed already in 1994 \cite{jose1994genetic}; this
survey focused on implementation aspects, but shows that most of them
used C or C++; Java was not even an option then. Implementation
...
where we could envision different parts of an application might be written in different
...
in a single language. However, these type of architectures have not
been found, so far, in the area of evolutionary algorithms. 

In this paper we have tried to extend the state of the art in the
measurements of performance of EAs in different languages by
performing the most extensive testing made so far. This test will
establish a baseline of measurement that can be used with new
languages and also for overall performance of particular operations in
...
Crossover is also part of that canonical EA, and MaxOnes or Count-Ones or OneMax is a fitness
...
list, string or vector processing. In general, copying and creating
...
Being as it is a loop, we should expect that the time needed would
grow linearly with the chromosome size. We will check whether this is
true or not for the different languages below. 
...
represent different philosophies in language design and also languages
traditionally used in the implementation of evolutionary algorithms
...
\begin{table*}[h!tb]
    ...
    \begin{tabular}{l|c|l|l|l}
      ...
      Clojure & 1.8.0 & \url{http://git.io/v8T57} & Persistent Vector &  Interpreted \\ 
      Common Lisp & 0.13.7 & \url{http://git.io/v8T57} & Simple Bit Vector &  Compiled\\ 
      Go & go1.2.1 & \url{http://git.io/vBSYp} & Bit Vector & Compiled\\
      Haskell & ghc 7.10.3 & \url{http://git.io/v8T57} & Mutable Vector & Compiled \\ 
      Java & 1.8.0\_66 & \url{http://git.io/v8TdR} & Bitset & Compiled\\
      JavaScript & node.js 5.0.0 & \url{http://git.io/vBSYd} & String & Interpreted\\
      Julia & 0.2.1 & \url{http://git.io/vBSOe} & Bit Vector & Compiled \\
...
%
    ...
    Compiled languages are represented by Rust, Scala, Java, Go, C and
...
usual functions found in evolutionary algorithm frameworks: mutation, crossover  % "found"  =>  "used"
and OneMax, for several compiled and interpreted languages with a wide  % instead of "onemax", better say "fitness function" (in line 165 it is presented as I propose)
range of use and popularity within the evolutionary computation and
scientific community and programmers at large.  
% and to determine how these programming languages affect the performance in EA experiments
...
a Docker container with all the tests so that it can be downloaded and % maybe we should include an URL to specify what Docker is
