...
This scenario is even more blurred if we look at the results for
crossover in Figure \ref{fig:time:xo}. The fastest overall are Go and,
...
for sizes bigger than 1024. The node JavaScript interpreter performs
...
counter. C is now the fastest, followed by node, and Lua the slowest,
